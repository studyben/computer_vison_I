...
class CrossEntropyLoss(BaseLayer):
    ...
    def forward(self, input_x: np.ndarray, target_y: np.ndarray):
        ...
        ones = np.zeros((target_y.size, target_y.max() + 1))
        ones[np.arange(target_y.size), target_y] = 1
        loss = np.sum(-np.log(np.sum(ones * input_x, axis=1)))
    ...
    def backward(self):
        ...
        ones = np.zeros((y_temp.size, y_temp.max() + 1))
        ones[np.arange(y_temp.size), y_temp] = 1
